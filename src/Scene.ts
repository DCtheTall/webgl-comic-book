...
export default class Scene {
  ...
  private readonly textureSrcs: Map<string, TexImageSource> = new Map();

  constructor(canvas: HTMLCanvasElement) {
    this.gl = canvas.getContext('webgl', {preserveDrawingBuffer: true})
      || canvas.getContext(
        'experimental-webgl', {preserveDrawingBuffer: true}) as WebGLRenderingContext;
  ...
  public addTexture(key: string, src: TexImageSource) {
    this.textureSrcs.set(key, src);
  ...
  private updateVideoTextures() {
    for (const [key, src] of this.textureSrcs) {
